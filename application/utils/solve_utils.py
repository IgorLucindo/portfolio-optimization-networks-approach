...
def solve_max_return(G, instance, delta=0.65, R_var=0.01, gamma=0.05):
    """
    ...
    # Return no solution if model is infeasible
...
def solve_max_return_cb(G, instance, delta=0.65, R_var=0.01, gamma=0.05):
    """
    MIP formulation for maximum mean return
    """
    (assets, daily_returns, min_daily_return, mean_return,
    ...
    # model.addConstr(gp.quicksum(z[t] for t in T_range) == round(delta * total_days), name="c1")
        ...
        if where == GRB.Callback.MIPSOL:
            ...
            for t in T_range:
                if z_[t] > 0.5 or sum(daily_returns[t, i] * x_[i] for i in G.nodes) >= R_var:
                    continue
                model.cbLazy(gp.quicksum(daily_returns[t, i] * x[i] for i in G.nodes) >= R_var - (R_var - min_daily_return[t]) * z[t])

    # Solve
    model.optimize(lazy_callback)
    print(callback_counter['count'])
...
def solve_max_return_cb2(G, instance, delta=0.65, R_var=0.01, gamma=0.05):
    """
    MIP formulation for maximum mean return
    """
    (assets, daily_returns, min_daily_return, mean_return,
